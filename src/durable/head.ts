...
/**
 * `engine` has no default. V1 opens an object only when the recorded engine
 * version is exactly the running one, and a head that records nothing cannot
 * satisfy that — so an absent block is a refusal to open, not a wildcard.
 */
function parseEngine(value: unknown): DurableEngineIdentity {
  ...
  return {
    ...
    version: nonEmptyString(value['version'], 'engine.version'),
  }
}
...
export function serializeHead(head: DurableHead, raw?: Record<string, unknown>): Uint8Array {
  ...
  mergeInto('engine', { name: head.engine.name, version: head.engine.version })
  mergeInto('lease', {
 ...
 */
export function coldHead(db: string, engineVersion: string): DurableHead {
  return {
